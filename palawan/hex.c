...
int print_hex_idx(BaseSequentialStream *chp,
              const void *block, int count, uint32_t start)
{
  return print_hex_offset(chp, block, count, 0, start);
}

int print_hex(BaseSequentialStream *chp, const void *block, int count)
{
  return print_hex_offset(chp, block, count, 0, 0);
}
